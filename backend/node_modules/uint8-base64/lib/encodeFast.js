...
exports.encodeFast = encodeFast;
const base64codes_1 = require("./base64codes");
/*
...
for (let i = 0; i < 64; i++) {
    for (let j = 0; j < 64; j++) {
        ...
        base64codes1[index] = base64codes_1.base64codes[i] | (base64codes_1.base64codes[j] << 8);
    }
}
...
for (let i = 0; i < 64; i++) {
    for (let j = 0; j < 64; j++) {
        const index = (i << 6) | j;
        base64codes2[index] = (base64codes_1.base64codes[i] << 16) | (base64codes_1.base64codes[j] << 24);
    }
}
/**
...
function encodeFast(input) {
    ...
    if (i === input.length + 1) {
        ...
        output32[j] =
            base64codes_1.base64codes[input[i - 2] >> 2] |
                (base64codes_1.base64codes[(input[i - 2] & 0x03) << 4] << 8) |
                (15677 << 16);
    }
    if (i === input.length) {
        ...
        output32[j] =
            base64codes_1.base64codes[input[i - 2] >> 2] |
                (base64codes_1.base64codes[((input[i - 2] & 0x03) << 4) | (input[i - 1] >> 4)] << 8) |
                (base64codes_1.base64codes[(input[i - 1] & 0x0f) << 2] << 16) |
                (61 << 24);
    }
    const output8 = new Uint8Array(output32.buffer);
    return output8;
}
//# sourceMappingURL=encodeFast.js.map