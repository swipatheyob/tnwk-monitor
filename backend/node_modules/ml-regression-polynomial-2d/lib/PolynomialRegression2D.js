...
exports.PolynomialRegression2D = void 0;
const ml_matrix_1 = require("ml-matrix");
const ml_regression_base_1 = require("ml-regression-base");
const BaseRegression2D_1 = require("./BaseRegression2D");
// Implements the Kernel ridge regression algorithm.
// http://www.ics.uci.edu/~welling/classnotes/papers_class/Kernel-Ridge.pdf
class PolynomialRegression2D extends BaseRegression2D_1.BaseRegression2D {
    order;
    coefficients;
    score;
    /**
    ...
    constructor(inputs, outputs, options = {}) {
        ...
        if (inputs === true) {
            // @ts-expect-error internal use only
            this.coefficients = ml_matrix_1.Matrix.columnVector(outputs.coefficients);
            // @ts-expect-error internal use only
            ...
            (0, BaseRegression2D_1.checkArrayLength)(inputs, outputs);
    ...
    _toFormula(precision, isLaTeX) {
        ...
        for (let i = 0; i <= this.order; i++) {
            for (let j = 0; j <= this.order - i; j++) {
                ...
                if (coefficient !== 0) {
                    str += (0, ml_regression_base_1.maybeToPrecision)(coefficient, precision);
                    if (i === 1) {
                        str += `${times}x`;
                    }
                    else if (i > 1) {
...
exports.PolynomialRegression2D = PolynomialRegression2D;
/**
...
function train(input, y, order) {
    if (!ml_matrix_1.Matrix.isMatrix(y))
        y = ml_matrix_1.Matrix.columnVector(y);
    const x = new ml_matrix_1.Matrix(y.rows, 2);
    ...
    const A = new ml_matrix_1.Matrix(examples, nbCoefficients);
    let col = 0;
    ...
    const svd = new ml_matrix_1.SVD(A.transpose(), {
        computeLeftSingularVectors: true,
        computeRightSingularVectors: true,
        autoTranspose: false,
    });
    let qqs = ml_matrix_1.Matrix.rowVector(svd.diagonal);
    qqs = qqs.apply((i, j) => {
    ...
    const qqs1 = ml_matrix_1.Matrix.zeros(examples, nbCoefficients);
...
//# sourceMappingURL=PolynomialRegression2D.js.map